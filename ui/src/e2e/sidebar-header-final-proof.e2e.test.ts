...
async function openProofState(state: ProofState) {
  ...
  await page.route(`**${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, (route) =>
    route.fulfill({
      json: {
        ...
        ...(state.environment
          ? { environment: state.environment, seamColor: "#f59e0b" }
          : {}),
      },
    }),
  );
  ...
  await card.waitFor();
  await expect.poll(() => card.locator(".sidebar-agent-card__name-text").textContent()).toBe(state.name);
  ...
  if (state.environment) {
    ...
    await expect.poll(() => card.locator(".sidebar-agent-card__avatar--environment").count()).toBe(1);
  }
...
suite.define(() => {
  it("captures the final-head sidebar header states", async () => {
    ...
    const states: ProofState[] = [
      { fileName: "01-light-short-unread-collapse-hover.png", hover: "collapse", name: "Molty", theme: "light", unread: true },
      { environment: { color: "amber", label: "Development" }, fileName: "02-light-long-environment-search-hover.png", hover: "search", name: longName, theme: "light" },
      { environment: { color: "amber", label: "Development" }, fileName: "03-dark-long-environment-unread-new-session-hover.png", hover: "new-session", name: longName, theme: "dark", unread: true },
      { fileName: "04-dark-short-menu-open.png", menuOpen: true, name: "Molty", theme: "dark" },
      { environment: { color: "amber", label: "Production — South America" }, fileName: "05-light-240-environment-unread.png", name: longName, theme: "light", unread: true, width: 240 },
      { environment: { color: "amber", label: "Production — South America" }, fileName: "06-dark-240-environment-unread.png", name: longName, theme: "dark", unread: true, width: 240 },
      { environment: { color: "amber", label: "Development" }, fileName: "07-light-browser-comfortable.png", name: "Molty", theme: "light" },
      { fileName: "08-dark-native-host-plus-right.png", name: "Assistant", nativeWebChrome: true, theme: "dark" },
      { environment: { color: "amber", label: "Development" }, fileName: "09-light-layout-guides.png", guides: true, name: "Molty", theme: "light" },
    ];
    ...
    for (const state of states) {
      ...
      try {
        if (state.width === 240) {
          ...
          await page.screenshot({ animations: "disabled", path: path.join(output, state.fileName) });
        }
      } finally {
        await context.close();
      }
