...
function renderMutationMessage(message: ModelProviderRowMessage | undefined) {
  ...
  return html`
    <div
      class="callout callout--${message.kind === "error" ? "danger" : "success"}"
      role=${message.kind === "error" ? "alert" : "status"}
    >
      ${message.text}
    </div>
    ${message.warning
      ? html`<div class="callout callout--warn" role="status">${message.warning}</div>`
      : nothing}
  `;
}
...
export function renderModelProviders(props: ModelProvidersViewProps) {
  ...
  return renderSettingsPage(html`
    ...
    ${props.mutationBlockedReason
      ? html`<div class="callout callout--warn">${props.mutationBlockedReason}</div>`
      : nothing}
  `);
}
