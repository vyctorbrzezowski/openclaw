...
export function renderMessageImages(images: RenderableImageBlock[], opts?: ImageRenderOptions) {
  ...
  const openImage = (img: RenderableImageBlock, previewUrl: string) => {
    ...
    if (!opts?.onOpenImage) {
      ...
      void resolveManagedOutgoingImageBlobUrl(img.displayUrl, opts, img.artifactId, "full")
        .then((freshUrl) => {
          ...
          if (!safeUrl) {
            pendingWindow?.close();
            showToast({ message: t("chat.imageLightbox.loadFailed"), tone: "danger" });
    ...
    void resolveManagedOutgoingImageBlobUrl(img.displayUrl, opts, img.artifactId, "full")
      ...
      .catch(() => showToast({ message: t("chat.imageLightbox.loadFailed"), tone: "danger" }));
  };
...
function renderManagedImageActions(
  ...
  const download = async () => {
    try {
      ...
      showToast({ message: t("chat.imageLightbox.downloadFailed"), tone: "danger" });
    }
  };
  const copy = async () => {
    try {
      ...
      showToast({ message: t("chat.imageLightbox.copyFailed"), tone: "danger" });
    }
  };
  return html`
