...
Chat keeps the composer focused on input. Session metadata and supporting work
such as environment, changes, pull requests, CI, goals, progress, and
sub-agents live in the **Session details** popover at the top right. Agent
messages, tool activity, approvals, questions, and collaboration activity stay
in the transcript.
