...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - Organization filters for `sessions.list` include nullable `category` (`null` means no custom category), `ownerId`, `unread`, run `status`, `projectId`, `hasWorktree`, and `needsAttention`. They are applied before sorting, pagination, row projection, title derivation, or transcript preview reads.
    - `sessions.search` searches the existing per-agent transcript FTS index. Omitted or `resultMode: "messages"` returns up to 25 message hits. `resultMode: "sessions"` groups by durable session generation before applying a limit of up to 100 and returns one best matching bounded excerpt per session without hydrating complete transcripts.
